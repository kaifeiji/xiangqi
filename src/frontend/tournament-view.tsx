...
export function TournamentView({ active, models, modelsLoaded }: TournamentViewProps): React.JSX.Element {
  ...
  return <section className="tournament-view" hidden={!active} aria-label="ELO 积分赛">
    <header className="tournament-header"><div><span className="eyebrow">MODEL LEAGUE</span><h2>ELO 积分循环赛</h2><p>每对模型进行 2 盘红黑交换对局。</p></div><div className="tournament-actions">{tournaments.length > 0 && <label>赛事<select value={tournamentId} onChange={(event) => void selectTournament(event.target.value)} disabled={creating}>{tournaments.map((tournament, index) => <option key={tournament.id} value={tournament.id}>第 {tournaments.length - index} 届</option>)}</select></label>}<label>MCTS<select value={simulations} onChange={(event) => setSimulations(Number(event.target.value))} disabled={creating || started}>{[0, 1000, 5000, 10000].map((value) => <option key={value} value={value}>{value} sims</option>)}</select></label>{!started && <button type="button" onClick={() => void start()} disabled={!modelsLoaded || creating || selected.length < 2}>{creating ? '创建中...' : '开始赛事'}</button>}{hasRunning ? <button className="secondary" type="button" onClick={() => void pauseAll()} disabled={creating}>暂停赛事</button> : hasPaused && <button className="secondary" type="button" onClick={() => void resumeAll()} disabled={creating}>继续赛事</button>}</div></header>
    <fieldset className="tournament-select" disabled={!modelsLoaded || creating || started}><legend>参赛模型</legend><div className="model-tags">{models.map((model) => <button className={`model-tag ${selected.includes(model.id) ? 'selected' : ''}`} key={model.id} type="button" aria-pressed={selected.includes(model.id)} onClick={() => toggleModel(model.id)}>{model.name}</button>)}</div><span className="selection-hint">已选择 {selected.length} 个模型{started ? ' · 赛事已开始，配置已锁定' : ''}</span></fieldset>
