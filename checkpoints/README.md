...
- `best.pt`：最低 `validation_j_select` 的 checkpoint，只保留当前一个。
- `best-policy.pt`：最低 `cp_policy_kl` 的 checkpoint，只保留当前一个。
- `best-value.pt`：最低 `value_cp_mae_le_300` 的 checkpoint，只保留当前一个。
